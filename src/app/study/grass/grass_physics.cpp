...
GrassPhysics::GrassPhysics(const std::vector<EdgeProperties> &edges): edges_static_(edges) {
    ...
    // Fill up properties
    CalcFixedProperties(props_, edges_static_);
    Reset();
}
...
void GrassPhysics::Update(tp::Real dt, const tp::Vec3 &wind_vec) {
    ...
    // Bend
    Rotation rotation_b = BendMotion(dt, wind_vec);
   // if (rotation_b.angle) {
    for (uint32_t i = 1; i < edges_static_.size(); ++i) {
        ...
        edges_.at(i).position = glm::rotate(edges_static_.at(i).position, current_angle, rotation_b.axis);
    }
   // }
    // Swing
    Rotation rotation_s = SwingMotion(dt, glm::rotate(wind_vec, -swing_.displacement_ang, util::math::kGroundNormal));
  //  if (rotation_s.angle) {
    for (uint32_t i = 1; i < edges_.size(); ++i) {  
        ...
        edges_.at(i).position = glm::rotate(edges_.at(i).position, current_angle, rotation_s.axis);
    }
  //  }
}

// Rotate always about x axis, adjust wind according to swing effect
GrassPhysics::Rotation GrassPhysics::BendMotion(tp::Real dt, const tp::Vec3 &wind_vec) {
    ...
    bend_.orientation.growth_vec = glm::rotate(props_.orientation.growth_vec, rotation.angle, rotation.axis);
    bend_.orientation.normal_vec = glm::rotate(props_.orientation.normal_vec, rotation.angle, rotation.axis);
...
GrassPhysics::Rotation GrassPhysics::SwingMotion(tp::Real dt, const tp::Vec3 &wind_vec) {
    ...
    // Update orientation vectors
    swing_.orientation.growth_vec = glm::rotate(props_.orientation.growth_vec, rotation.angle, rotation.axis);
    swing_.orientation.side_vec = glm::rotate(props_.orientation.side_vec, rotation.angle, rotation.axis);
...
void GrassPhysics::CalcFixedProperties(Properties &props, const std::vector<EdgeProperties> &edges) {
    ...
    CalcSimVectors(props.orientation, edges);
}

// Calculate growth normal and side vectors from given edges
void GrassPhysics::CalcSimVectors(Orientation &orient, const std::vector<EdgeProperties> &edges) {
